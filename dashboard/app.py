...
<div class="rm-hero">
  <div class="rm-hero-meta">
    ...
    <span class="rm-nav-edition">2025 SEASON</span>
  </div>
...
def _get_driver_models(driver_code):
    ...
    m1 = get_compound_degradation(laps, c1, race_laps=race_laps)
    m2 = get_compound_degradation(laps, c2, race_laps=race_laps)
    if m1 is None or m2 is None:
        return None, None, None, None
    return c1, c2, m1, m2


compound_1, compound_2, model_1, model_2 = _get_driver_models(selected_driver)
...
if model_1 is None or model_2 is None:
    ...
    for d in drivers:
        if d == selected_driver:
            continue
        c1, c2, m1, m2 = _get_driver_models(d)
        if m1 and m2:
            fallback_driver = d
...
simulation_results = simulate_strategy_window(
    PIT_WINDOW_MIN, PIT_WINDOW_MAX, model_1, model_2, race_laps
)
...
tab_overview, tab_strategy, tab_degradation, tab_comparison = st.tabs([
    "OVERVIEW", "STRATEGY", "DEGRADATION", "COMPARISON"
])
...
with tab_overview:
    ...
    with left:
        with st.container(border=True):
            ...
            <h1 class="rm-driver-title" style="color:var(--red);">{selected_driver}</h1>
            <p class="rm-driver-sub">🏁 {race_laps} LAPS</p>
            """, unsafe_allow_html=True)
            ...
            with col_a:
                ...
                <div class="rm-compound-stat">
                  ...
                  <span class="rm-compound-sub">R² {model_1["R2"]:.3f}</span>
            ...
            with col_b:
                ...
                <div class="rm-compound-stat">
                  ...
                  <span class="rm-compound-sub">R² {model_2["R2"]:.3f}</span>
                </div>""", unsafe_allow_html=True)

    with mid:
        delta_label = f"+{time_delta:.2f}s vs optimal" if time_delta > 0.5 else "Optimal ✓"
        st.metric("Predicted Race Time", f"{predicted_time:.2f}s", delta=delta_label, delta_color="inverse")
    ...
    st.caption(f"IQR-filtered linear regression · FIA fuel correction · Pit delta: {PIT_STOP_DELTA:.0f}s")
...
with tab_strategy:
    ...
    fig.add_trace(go.Scatter(
        x=simulation_df["PitLap"], y=simulation_df["PredictedRaceTime"],
        mode="lines+markers", name="Predicted Race Time",
        line=dict(color=accent, width=3), marker=dict(size=7, color=accent),
    ))
    fig.add_trace(go.Scatter(
        x=[pit_lap], y=[predicted_time], mode="markers",
        name=f"Selected (Lap {pit_lap})",
        marker=dict(size=14, color=accent, symbol="diamond", line=dict(color="white", width=2)),
    ))
    fig.add_trace(go.Scatter(
        x=[best_pit_lap], y=[best_time], mode="markers",
        name=f"Optimal (Lap {best_pit_lap})",
        marker=dict(size=14, color="#22C55E", symbol="star", line=dict(color="white", width=1.5)),
    ))
    fig.add_vline(
        x=best_pit_lap, line_width=2, line_dash="dash", line_color="#22C55E",
    ...
    if not candidates.empty:
        fig.add_vrect(
            x0=candidates["PitLap"].min(), x1=candidates["PitLap"].max(),
            fillcolor="#22C55E", opacity=0.08, line_width=0,
            annotation_text="±1s window", annotation_position="top left",
            annotation_font_size=10, annotation_font_color="#22C55E",
        )
    if pit_lap != best_pit_lap:
        fig.add_vline(
            x=pit_lap, line_width=2, line_dash="dot", line_color=accent,
    ...
    fig.update_layout(
        **PLOT_LAYOUT,
        ...
        xaxis_title="Pit Stop Lap", yaxis_title="Predicted Race Time (s)",
    ...
    if race_laps >= 50:
        ...
        if "Error" not in stop_comparison:
            ...
            if abs(gain) > 3:
                st.success(f"**{stop_comparison['Recommended']} strategy recommended** — saves **{abs(gain):.1f}s**")
            elif abs(gain) > 1:
    ...
    recommendation  = build_strategy_recommendation(
        selected_driver, pit_lap, best_pit_lap,
        predicted_time, best_time, model_1, model_2,
    )
    actual_vs_optimal = compare_actual_vs_optimal(
        driver_laps, model_1, model_2, race_laps, PIT_WINDOW_MIN, PIT_WINDOW_MAX
    )

    with st.container(border=True):
        st.markdown(f"""<div class="rm-card-header">
          <span class="rm-card-eyebrow">STRATEGY VERDICT</span>
    ...
    uc1.metric("Undercut Status",  "VIABLE" if undercut["Viable"] else "MARGINAL")
    uc2.metric("Gain per Lap",     f"{undercut['ProjectedGainPerLap']:+.3f}s")
    uc3.metric("Net Gain (3 laps)", f"{undercut['TotalProjectedGain']:+.3f}s")
    with st.container(border=True):
...
with tab_degradation:
    ...
    st.info("Lap times are IQR-filtered to remove safety-car and VSC laps. FIA fuel correction applied.")
    ...
    with deg_tab1:
        ...
        degradation_data = [
            {
                "TyreLife": tl,
                "LapTime":  model["Intercept"] + model["Slope"] * tl,
                "Compound": compound,
            }
            for tl in range(1, max_tyre_life + 1)
        ...
        for compound in [compound_1, compound_2]:
            ...
            fig2.add_trace(go.Scatter(
                x=cdata["TyreLife"], y=cdata["LapTimeSeconds"],
                mode="markers", name=f"{compound} (actual)",
                marker=dict(color=c_color, size=6, opacity=0.4),
            ))
            fig2.add_trace(go.Scatter(
                x=mdata["TyreLife"], y=mdata["LapTime"],
                mode="lines", name=f"{compound} model ({model['Slope']:+.3f}s/lap)",
        ...
        fig2.update_layout(
            **PLOT_LAYOUT,
            ...
            xaxis_title="Tyre Life (laps)", yaxis_title="Lap Time (s)",
        ...
        st.markdown("""<div class="rm-card-header">
          <span class="rm-card-eyebrow">MODEL NOTES</span>
          <span class="rm-card-tag">LINEAR REGRESSION</span>
        </div>""", unsafe_allow_html=True)
        st.caption(
            "**Why linear?** Pirelli degradation is approximately linear until the performance cliff "
            "(last 3–5 laps). Linear regression avoids overfitting and gives interpretable s/lap rates.\n\n"
            "**R² guide:** >0.7 Strong · 0.3–0.7 Moderate · <0.3 Noisy\n\n"
            f"R² — {compound_1}: **{model_1['R2']:.3f}** · {compound_2}: **{model_2['R2']:.3f}**"
        )

    with deg_tab2:
        stint_summary = get_stint_summary(driver_laps, race_laps=race_laps)
...
with tab_comparison:
    ...
    with st.spinner("Simulating all drivers…"):
        ...
        comparison_df = rank_drivers_fair(
            session, drivers, _get_driver_laps_cached,
            get_compound_degradation, race_laps,
            PIT_WINDOW_MIN, PIT_WINDOW_MAX, fastest_driver,
        )
    ...
    with st.container(border=True):
        ...
        st.caption(f"**{winner['Driver']}** — {winner['Compound1']} → {winner['Compound2']}, pit lap {int(winner['OptimalPitLap'])}")
    ...
    for i, col in enumerate(podium_cols):
        ...
        with col:
            with st.container(border=True):
                st.markdown(
                    f"{'P1 · P2 · P3'.split(' · ')[i]} "
                    f"<span style='color:{drv_color}; font-weight:700;'>{row['Driver']}</span>",
                    unsafe_allow_html=True,
                )
    ...
    fig3 = px.scatter(
        comparison_df,
        x="OptimalPitLap", y="PredictedTime",
        color="Driver", color_discrete_map=DRIVER_COLORS, text="Driver",
        title=f"{selected_race} — Strategy Comparison",
        hover_data={
            "Driver": True, "OptimalPitLap": True,
            "PredictedTime": ":.2f", "DeltaToLeader": ":.2f",
            "Compound1": True, "Compound2": True,
        },
    )
    fig3.update_traces(
    ...
    fig3.update_layout(
        **PLOT_LAYOUT,
        xaxis_title="Optimal Pit Lap", yaxis_title="Predicted Race Time (s)",
        xaxis=dict(showgrid=False),
        yaxis=dict(gridcolor="#1A1A1A"),
        showlegend=False,
    )
    ...
    display_df.columns = [
        ...
        "Δ Gap (s)", "Compound 1", "Compound 2", "Deg 1 (s/lap)", "Deg 2 (s/lap)",
    ]
    st.dataframe(
        display_df.style
            .format({
    ...
    st.download_button(
        "EXPORT DATA", csv,
        f"racemind_{selected_race.replace(' ', '_')}_strategy.csv",
        "text/csv",
    )